...
#execute as @s[scores={inven_tab-info=1}] at @e[tag=modify-head] as @a if score @e[distance=..0,limit=1,sort=nearest] class-index = @s class-index run function comm:about-play/arrow-setting/hotbar
execute as @s[scores={inven_tab-info=1}] at @e[tag=modify-head] as @a if score @e[distance=..0,limit=1,sort=nearest] class-index = @s class-index run loot replace block ~ 21 ~ container.8 mine ~ 17 ~ stick{drop_contents:true}

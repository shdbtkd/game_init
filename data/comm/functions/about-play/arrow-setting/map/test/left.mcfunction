...
execute at @e[tag=modify-head] as @s if score @e[distance=..0,limit=1,sort=nearest] class-index = @s class-index store result block ~ 17 ~ Items[{ Slot: 12b }].tag.CustomModelData int 1 run scoreboard players get @s map_type
...
execute at @e[tag=modify-head] as @s if score @e[distance=..0,limit=1,sort=nearest] class-index = @s class-index store result block ~ 17 ~ Items[{ Slot: 11b }].tag.CustomModelData int 1 run scoreboard players get @s map_type

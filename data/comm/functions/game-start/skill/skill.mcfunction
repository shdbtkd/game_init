...
execute as @a store result score @s jewelry_inven if data entity @s Inventory[{ Slot: 4b, tag: { jewelry_item: 1b } }]
...
#execute at @e[tag=modify-head] as @a if score @e[distance=..0,limit=1,sort=nearest] class-index = @s class-index if data block ~ 27 ~ Items[{ Slot: 0b }] unless data block ~ 15 ~ Items[{ Slot: 2b }] run function comm:game-start/skill/loot_to_weapon
#execute at @e[tag=modify-head] as @a if score @e[distance=..0,limit=1,sort=nearest] class-index = @s class-index if data block ~ 27 ~ Items[{ Slot: 1b }] unless data block ~ 15 ~ Items[{ Slot: 3b }] run function comm:game-start/skill/loot_to_weapon
#execute at @e[tag=modify-head] as @a if score @e[distance=..0,limit=1,sort=nearest] class-index = @s class-index if data block ~ 27 ~ Items[{ Slot: 2b }] unless data block ~ 15 ~ Items[{ Slot: 4b }] run function comm:game-start/skill/loot_to_weapon
#execute at @e[tag=modify-head] as @a if score @e[distance=..0,limit=1,sort=nearest] class-index = @s class-index if data block ~ 27 ~ Items[{ Slot: 3b }] unless data block ~ 15 ~ Items[{ Slot: 5b }] run function comm:game-start/skill/loot_to_weapon

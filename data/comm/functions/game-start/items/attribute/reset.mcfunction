...
execute at @e[tag=modify-head] as @a if score @e[distance=..0,limit=1,sort=nearest] class-index = @s class-index run data modify block ~ 27 ~ Items[{ Slot: 3b }].tag.AttributeModifiers[{ AttributeName: "generic.attack_damage" }].Amount set value 0